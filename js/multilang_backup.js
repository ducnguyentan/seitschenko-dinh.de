...
function updateLanguageDisplay() {
  ...
  if (currentLangSpan) {
    currentLangSpan.textContent = config.code;
  }
  if (currentFlag) {
  ...
  if (mobileLangSpan) {
    mobileLangSpan.textContent = config.code;
  }
  if (mobileFlag) {
    ...
    try {
      ...
      document.querySelectorAll('[data-translate]').forEach(element => {
        ...
        if (value) {
          ...
          // Silently skip missing translation keys to avoid console spam
          // Translation keys should be defined in page-specific translations
        }
      });
...
function initializeMultilingual(pageTranslations = {}) {
  ...
  // Ensure current language is supported
